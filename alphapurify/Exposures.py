...
class PortfolioExposures():
    ...
    def calc_stats(self):
        ...
        result_df = result_df.fill_null(0).fill_nan(0)
        ...
        result_df = (
            ...
            .with_columns([
                pl.col(f"{f}_attr").fill_nan(0)
                for f in self.exposure_cols
            ])
            .with_columns(
                [
                    (
                        (1 + pl.col(f"{f}_attr").fill_nan(0))
                        .cum_prod()
                        - 1
                    ).alias(f"{f}_cum_ret")
                    for f in self.exposure_cols
    ...
    def calc_stats(self):
        ...
        df = df.with_columns(
                (
                    ...
                    / pl.col(self.factor_name).abs().sum().over(self.trade_date_col)
                ).alias("weight")
                )
        ##########################
        mean_df = (
